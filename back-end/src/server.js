...
app.post("/v1/Comment/Post", async(req, res) => {
    ...
    console.log(Comment);
    try {
        await admin.firestore().collection("Comment").add(Comment).then(doccomment => {
            admin.firestore().collection("videos").doc(req.body.vid).update({
                "comments": admin.firestore.FieldValue.arrayUnion(doccomment.id)
            })
        })
    } catch (err) {
...
>>> > 75e a2b58c4892b8f8c74b9bcb48718684fd6d23b